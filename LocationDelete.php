...
<?php
	...
	include ("connect.php");
		...
		$Loc_IdDelCode=$_GET["Loc_Id"];
   		$Loc_Id=base64_decode(base64_decode("$Loc_IdDelCode"));

		$sqlchk = "select * from AssetIT where Loc_Id = '$Loc_Id'";
	$querychk = mssql_query($sqlchk);
	$num = mssql_num_rows($querychk);
	if($num>0)
	{
		echo"<script>alert('Can not delete data Because the data is correlated with Asset IT information.');history.back();</script>";
		exit();
	}	
