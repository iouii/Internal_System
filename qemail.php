...
include "connect.php";
    ...
    $EType= $_POST["EType"];
    if ($EType=="e-mail"){ // select type  email
    $i=1;
    $Nation = $_POST["ENaId"];
//--copy all e-mail where nationality select-----------
...
if($Nation == "Thai"){
    ...
    mssql_close();
  //-----------------------------------------------------------  
  //get value from email-index.php
    $EPosId = array();
    $ENaId= $_POST["ENaId"];
    $EDepId= $_POST["EDepId"];
...
//get multi value split text and insert "  ,  " 
...
//check data if user select multi position
    if($EChk == "cb"){
...
//check data if user select multi department
...
if(mssql_num_rows($query) <1){
   ...
   <TD colspan=12><div align='center' class='style82'><br><IMG SRC=images/icon/Nodata.png border=0 WIDTH=100 HEIGHT=100 align=absmiddle>
    ...
    <td  align='center'>
    Send Select E-Mail
    <img src='Img\outlook64.png' title='Send Select E-Mail'  id='button' style='width:45px !important;cursor:pointer;  '> 
    </td>
...
?>
    ...
    <?}?>
    <?
if($EType=="te-lephone"){ //select type telephone
    $i= 1;
    ...
    <table class='table the'>
        ...
        <tr>
        <td align='right'>Export To CSV <a href=$filName> <img src='Img/excel64.png' style='width:45px !important ' title='Export Telephone To Excel ' ></a>
...
while($qry=mssql_fetch_array($query)){ 
    ...
    <?echo"<td align='center'> $i </td>"; ?>
    <?$i++; ?>
    <? echo"<td align='center'class='color-ex'><b>$Emp_ExtensionNumber</b></td>"; ?>
    <? echo"<td align='center'class='color-ex'>$Emp_SpeedDial</td>"; ?>
    <? echo"<td align='center'class='color-ex'>$Emp_Mobile</td>"; ?>
    <? echo"<td>$Emp_NickName</td>"; ?>
    <? echo"<td>$Emp_Name</td>"; ?>
    <? echo"<td>$Dep_Name</td>"; ?>
    <? echo"<td>$Pos_Name</td>"; ?>
...
<script>
 function toggle(it) {
    ...
    // toggle chang background color ** data table
...
<script>
// function copy email get value send email
function copymail() {
...
<script type="text/javascript"> // send email where select but  don't 
$(document).ready(function() {
    $("#button").click(function() {
        ...
        if (maill == "") {
          ...
          var mail = maill.toString();

          var res = mail.replace(/,/g, ";");
